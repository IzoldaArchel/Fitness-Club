<?php
/**
 * @file supressionActivite.php
 * @author Kolomiets Elena
 * @version 1.0
 * @date xx Janvier 2019
 * @brief La page qui permet à l'administrateur de supprimer une activité.
*/
...
if($confirme !== "") {
    ...
    if($confirme === "OUI") {
        ...
        $codRet = supprimerActivite($conn, $id);
        if($codRet === 1)  {
            $message = "Suppression effectuée.<br><h2>Pour effectuer une nouvelle suppression veiller retourner aux <a href='admin.php'>activités</a></h2>";
            $succes = 1;
                                 }
        elseif($codRet === 0)  
...
<body>
    ...
    <main>
        ...
        <section>
            <?php if(!$succes) : ?>
            <p>Confirmez la suppression d'une activité :</p>
            <form action="supressionActivite.php" method="post"> 
